 ...
 */
function loadSceneData(uuid: string) {
    var allData = getAllData();
 ...
 * @param {string} uuid - 场景/预制体 UUID
 */
function applySceneData(uuid: string) {
...
module.exports = {
    ...
    messages: {
        ...
        // 标尺显示状态改变
        'visible-changed'(event: any, visible: boolean) {
        ...
        'add-guide'(event: any, guide: any) {
            sendToRenderer('design-ruler:add-guide', guide);
        },

        'remove-guide'(event: any, index: any) {
        ...
        'set-design-image'(event: any, base64: string, filePath: string) {
            // 保存文件路径用于持久化
            if (filePath) {
                _currentDesignImagePath = filePath;
                saveCurrentData();
            }
            Editor.Scene.callSceneScript(PACKAGE_NAME, 'set-design-image', base64, (err: any) => {
                if (err) Editor.error('[design-ruler] set-design-image error:', err);
            });
        },
        ...
        // 保存资源拷贝规则
        'save-copy-rules'(event: any, rulesJson: string) {
            try {
        ...
        // 浏览目录
        'browse-directory'(event: any, data: string) {
            try {
        ...
        // 浏览根目录
        'browse-root-dir'(event: any, data: string) {
            try {
                var dataObj = JSON.parse(data);
        ...
        // 保存根目录配置
        'save-root-dirs'(event: any, configJson: string) {
            try {
        ...
        // 拷贝单个资源
        'copy-single-resource'(event: any, ruleJson: string) {
            try {
        ...
        // 拷贝所有资源
        'copy-all-resources'(event: any, rulesJson: string) {
            try {
                var rules = JSON.parse(rulesJson);
        ...
        // 刷新meta文件
        'refresh-meta-files'(event: any, dirPath: string) {
            try {
        ...
        // 打开源目录
        'open-source-dir'(event: any, data: string) {
            try {
        ...
        // 打开目标目录
        'open-target-dir'(event: any, dirPath: string) {
            try {
        ...
        // 清空目标目录
        'clear-target-dir'(event: any, dirPath: string) {
            try {
