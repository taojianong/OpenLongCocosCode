...
const FileUtil = {
    ...
    async copy(srcPath, destPath) {
        ...
        } else if (stats.isFile()) { //复制文件

            const dir = Path.dirname(destPath);
            if (!Fs.existsSync(dir)) {
                Fs.mkdirSync(dir, { recursive: true });//递归创建 
            }
            const url = await FileUtil.readFile(srcPath);
            //destPath 必须为完整文件路径
            await FileUtil.writeFile(destPath, url);
    ...
    async delete(path) {
        ...
        if (stats.isDirectory()) {
            ...
            // }
            // Fs.rmdirSync(path); //只能删除空目录

            try {
                Fs.rmSync(path, { recursive: true, force: true });//可删除一个非空目录
                // console.log(`目录 ${path} 已成功删除`);
            } catch (error) {
