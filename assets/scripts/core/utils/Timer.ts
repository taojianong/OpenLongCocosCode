...
export default class Timer {
    ...
    public _update(dt: number): void {
        ...
        var frame: number = this.currFrame = this.currFrame + 1;

        this._delta = dt;
        var timer: number = this.currTimer = this.currTimer + this._delta;
        this._lastTimer = timer;
        var awake: boolean = (this.currTimer - this._lastTimer) > 30;
    ...
    private _indexHandler(handler: TimerHandler): void {
        ...
        var cid: number = caller ? caller.$_GID || (caller.$_GID = guid++) : 0;
